using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;

namespace JewelsFeedTracker.Data.Models.Models
{
    // Root myDeserializedClass = JsonConvert.Deserializestring<Root>(myJsonResponse); 
    public class JbBrother
    {
        public string SrNo { get; set; }
        public string RefNo { get; set; }
        public string Shape { get; set; }
        public string Availability { get; set; }
        public string Carat { get; set; }
        public string Price { get; set; }
        public string RapOff { get; set; }
        public string RapaportPrice { get; set; }
        public string Lab { get; set; }
        public string CertNo { get; set; }
        public string Color { get; set; }
        public string CS { get; set; }
        public string Purity { get; set; }
        public string Cut { get; set; }
        public string stringensity { get; set; }
        public string Overtone { get; set; }
        public string FancyColor { get; set; }
        public string Symmetry { get; set; }
        public string Polish { get; set; }
        public string FL { get; set; }
        public string Luster { get; set; }
        public string TI { get; set; }
        public string BIS { get; set; }
        public string BIC { get; set; }
        public string OPPV { get; set; }
        public string OPCR { get; set; }
        public string OPTA { get; set; }
        public string EyeClean { get; set; }
        public string HA { get; set; }
        public string Culate { get; set; }
        public string EFCR { get; set; }
        public string EFPV { get; set; }
        public string TOI { get; set; }
        public string GirdleCondition { get; set; }
        public string FluoresentColor { get; set; }
        public string BowTie { get; set; }
        public string TabelPer { get; set; }
        public string Girdle { get; set; }
        public string CrownAngle { get; set; }
        public string CrownHeight { get; set; }
        public string PavalionHeight { get; set; }
        public string PavalionAngle { get; set; }
        public string TotalDept_mm { get; set; }
        public string TotalDepthPer { get; set; }
        public string DimMinAndWidth { get; set; }
        public string DimMaxAndWidth { get; set; }
        public string Ratio { get; set; }
        public string KeytoSymbols { get; set; }
        public string ReportComments { get; set; }
        public string CertiLink { get; set; }
        public string StoneImageLink { get; set; }
        public string Stone360Link { get; set; }
        public string TotalAmount { get; set; }
        public string Measurements { get; set; }
        public string RoughOrigin { get; set; }
    }


}
