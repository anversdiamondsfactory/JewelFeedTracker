...
{
    //  GlowStar Feed RAW Data Processing Logic with saving locally, manipulting with business rules applied and execution of Bulk Copy
    // command after a well-defined format in price & pricedescription tables....
    public class GlowStarFactory<PKTDTL> : FeedFactory<PKTDTL>
    {
        ...
        private Task<bool> DataBusinessRulesOnFeed(DataTable dtTarget)
        {
            try
            {
                ...
                if (!System.IO.Directory.Exists(path[0] + "\\stone_feed\\" + FeedIdentifier.Glowstar.ToString() + "\\"))
                    System.IO.Directory.CreateDirectory(path[0] + "\\stone_feed\\" + FeedIdentifier.Glowstar.ToString() + "\\");
                string file_path = path[0] + "FeedArchieves\\" + "stone_feed\\" + FeedIdentifier.Glowstar.ToString() + "\\";
                ...
                var fluorescence_arr = GlowStarArray.GetFluorescence();
                var polish_arr = GlowStarArray.GetPolish();
                var symmetry_arr = GlowStarArray.GetSymmetry();
                var cut_arr = GlowStarArray.GetCut();
                var lab_array = GlowStarArray.GetLab();
                var color_array = GlowStarArray.GetColor();
                ...
                if (isFileExist && linecount > 1)
                {
                    ...
                    for (int i = 0; i < dtTarget.Rows.Count; i++)
                    {
                        ...
                        if (!string.IsNullOrEmpty(dtTarget.Rows[i][35].ToString()) && dtTarget.Rows[i][35].ToString().ToLower() == "in lab") { continue; }

                        if (!string.IsNullOrEmpty(dtTarget.Rows[i][35].ToString()) && dtTarget.Rows[i][35].ToString() == "France") { dtTarget.Rows[i][35] = "France, Metropolitan"; }

                        if (!string.IsNullOrEmpty(dtTarget.Rows[i][19].ToString()) && dtTarget.Rows[i][19].ToString().ToUpper() == "SGL") { dtTarget.Rows[i][19] = "HRD"; }
                        ...
                        if (!string.IsNullOrEmpty(shape) && shape == "RND")
                        {
                            ...
                            {
                                //cut_grade = findStoneCut($rules, $shape, $polish, $symmetry, $data[13], $data[14]);
                        ...
                        if (int.Parse(dtTarget.Rows[i][17].ToString()) > 0 && int.Parse(dtTarget.Rows[i][18].ToString()) > 0) { }
                        else
                        {
                            continue;
                        }
                        ...
                        if (!string.IsNullOrEmpty(countries_list[dtTarget.Rows[i][35].ToString()]))
                        {
                            country = int.Parse(countries_list[countries_list[dtTarget.Rows[i][35].ToString()]]);
                        }
                        else
                        {
                        ...
                        string measurement = dtTarget.Rows[i][10].ToString() + " x " + dtTarget.Rows[i][11].ToString() + " x " + dtTarget.Rows[i][12].ToString();

                        int measlength = 0;
                        int measwidth = 0;
                        int measdepth = 0;
                        int ratio = 0;
                        measlength = int.Parse(dtTarget.Rows[i][10].ToString());
                        measwidth = int.Parse(dtTarget.Rows[i][11].ToString());
                        measdepth = int.Parse(dtTarget.Rows[i][12].ToString());
                        ratio = measlength / measwidth;

                        string cert = dtTarget.Rows[i][20].ToString();
                        string stock = "";
                        string available = "";
                        string depth = dtTarget.Rows[i][13].ToString();
                        string table_table = dtTarget.Rows[i][14].ToString();
                        string girdle = dtTarget.Rows[i][32].ToString();
                        string culet = dtTarget.Rows[i][33].ToString();
                        string culet_size = "";
                        ...
                        string image_url = "";
                        string video_url = !string.IsNullOrEmpty(dtTarget.Rows[i][37].ToString()) ? dtTarget.Rows[i][37].ToString() : "";
                        string packetID = "";
                        ...
                        _drPrice1_description["source"] = "GLOWST"; // source
