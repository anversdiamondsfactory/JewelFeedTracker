...
{
    // Akarsh Feed RAW Data Processing Logic with saving locally, manipulting with business rules applied and execution of Bulk Copy
    // command after a well-defined format in price & pricedescription tables....
    public class AkarshFactory<Row> : FeedFactory<Akarsh>
    {
        ...
        public async override Task GetFeedData(string RawDataUrl) 
        {
            ...
            try
            {
                ...
                string fileName = DataFormatter.SetFeedFileName(FeedIdentifier.Akarsh.ToString(), 'R');
                ...
                if (DataFormatter.IsValidJson(jsonString))
                {
                    ...
                    DataTable dtTarget = (DataTable)JsonConvert.DeserializeObject(jsonString, (typeof(DataTable)));
                    dtTarget.TableName = "AKARSH_MAST";                   
                    ...
                    await DataBusinessRulesOnFeed(dtTarget); // Business rules execution logic on Raw Data
                     if (dtPrice1 != null && dtPrice1.Rows.Count > 0)
                        await iFeedQueryProcessor.SaveFeed(dtPrice1, FeedIdentifier.Akarsh.ToString());  // Bulk data processing on stone_price1 Table
                    if (dtPrice1_description != null && dtPrice1_description.Rows.Count > 0)
                        await iFeedQueryProcessor.SaveFeed(dtPrice1_description, FeedIdentifier.Akarsh.ToString());  // Bulk data processing on Stone_price1_description Table

                    //JObject o = JObject.Parse(jsonString);
                    //JArray a = (JArray)o["Rows"];
                    //akarshList = a.ToObject<List<Akarsh>>();// Convert to List Data of raw data datatable
                }
                //DataFormatter.ExportCsv(akarshList, DataFormatter.SetFeedFileName(FeedIdentifier.Akarsh.ToString(), 'F'));

            }
            catch (Exception ex)
            {
                Log.Error("exception is occurred in " + FeedIdentifier.Akarsh.ToString(), ex.ToString());
            }
            //return akarshList;
        }
        private Task<bool> DataBusinessRulesOnFeed(DataTable dtTarget)
        {
            try
            {
                ...
                if (!System.IO.Directory.Exists(path[0] + "\\stone_feed\\" + FeedIdentifier.Akarsh.ToString() + "\\"))
                    System.IO.Directory.CreateDirectory(path[0] + "\\stone_feed\\" + FeedIdentifier.Akarsh.ToString() + "\\");
                string file_path = path[0] + "FeedArchieves\\" + "stone_feed\\" + FeedIdentifier.Akarsh.ToString() + "\\";
                ...
                var fluorescence_arr = AkarshArray.GetFluorescence();
                var polish_arr = AkarshArray.GetPolish();
                var symmetry_arr = AkarshArray.GetSymmetry();
                var cut_arr = AkarshArray.GetCut();

                int stone_vendor_id = 32;
                ...
                if (isFileExist && linecount > 1)
                {
                    ...
                    for (int i = 0; i < dtTarget.Rows.Count; i++)
                    {
                        ...
                        if (!string.IsNullOrEmpty(dtTarget.Rows[i][38].ToString()) && dtTarget.Rows[i][38].ToString() == "HK") { dtTarget.Rows[i][38] = "Hong Kong"; }

                        if (!string.IsNullOrEmpty(dtTarget.Rows[i][38].ToString()) && dtTarget.Rows[i][38].ToString() == "INDIA") { dtTarget.Rows[i][38] = "India"; }

                        if (!string.IsNullOrWhiteSpace(dtTarget.Rows[i][2].ToString()) && dtTarget.Rows[i][2].ToString().ToUpper() == "SGL") { dtTarget.Rows[i][2] = "HRD"; }

                        if (!(lab_array.ContainsKey(dtTarget.Rows[i][2].ToString()))) { continue; }
                        if (!color_array.Contains(dtTarget.Rows[i][9].ToString())) { continue; }

                        if ((string.IsNullOrEmpty(dtTarget.Rows[i][8].ToString()) || double.Parse(dtTarget.Rows[i][8].ToString()) < 0.20) ||
                            (!string.IsNullOrEmpty(dtTarget.Rows[i][16].ToString()) && (int.Parse(dtTarget.Rows[i][16].ToString()) >= 66))) { continue; }

                        string stone = "DI";

                        if (cut_arr.ContainsKey(dtTarget.Rows[i][11].ToString()))
                        {
                            dtTarget.Rows[i][11] = cut_arr[dtTarget.Rows[i][11].ToString()];
                        }

                        if (polish_arr.ContainsKey(dtTarget.Rows[i][12].ToString()))
                        {
                            dtTarget.Rows[i][12] = polish_arr[dtTarget.Rows[i][12].ToString()];
                        }

                        if (symmetry_arr.ContainsKey(dtTarget.Rows[i][13].ToString()))
                        {
                            dtTarget.Rows[i][13] = symmetry_arr[dtTarget.Rows[i][13].ToString()];
                        }
                        if (fluorescence_arr.ContainsKey(dtTarget.Rows[i][14].ToString()))
                        {
                            dtTarget.Rows[i][14] = fluorescence_arr[dtTarget.Rows[i][14].ToString()];
                        }

                        string shape = string.Empty;
                        if (!string.IsNullOrEmpty(mapping[dtTarget.Rows[i][7].ToString()]) && mapping[dtTarget.Rows[i][7].ToString()] == null)
                        {
                            shape = mapping[dtTarget.Rows[i][7].ToString()].ToUpper();
                            if (!string.IsNullOrEmpty(options["stone_shape"] + shape[0]) && options["stone_shape"] + shape[0] != null)
                            {
                                shape = options["stone_shape"] + shape[0];
                            }
                        }
                        else if (!string.IsNullOrEmpty(options["stone_shape"] + dtTarget.Rows[i][7].ToString().Trim()[0]) && options["stone_shape"] + dtTarget.Rows[i][7].ToString().Trim()[0] == null)
                        {
                            shape = options["stone_shape"] + dtTarget.Rows[i][7].ToString().Trim()[0];
                        }
                        else
                        {
                            continue;
                        ...
                        string weight = dtTarget.Rows[i][8].ToString();
                        Dictionary<string, int> crt_range = new Dictionary<string, int>();
                        if (!string.IsNullOrEmpty(mapping[dtTarget.Rows[i][8].ToString()]) && mapping[dtTarget.Rows[i][8].ToString()] == null)
                        {
                        ...
                        string color = dtTarget.Rows[i][9].ToString();
                        string intensity = "";
                        ...
                        if (!string.IsNullOrEmpty(dtTarget.Rows[i][10].ToString()) && (!string.IsNullOrEmpty(options["stone_clarity"] + dtTarget.Rows[i][10].ToString()[0]))) //  options["stone_clarity"] + dtTarget.Rows[i][5].ToString()[0]) != null)
                        {
                            clarity = options["stone_clarity"] + dtTarget.Rows[i][10].ToString()[0];
                        }
                        else
                        {
                            continue;
                        ...
                        if (!string.IsNullOrEmpty(dtTarget.Rows[i][12].ToString()) && !string.IsNullOrEmpty(options["stone_polish"]) && options[dtTarget.Rows[i][12].ToString()] == null)
                        {
                            polish = options["stone_polish"] + dtTarget.Rows[i][12].ToString()[0];
                        }
                        else
                        {
                            continue;
                        ...
                        if (!string.IsNullOrEmpty(dtTarget.Rows[i][13].ToString()) && !string.IsNullOrEmpty(options["stone_symmetry"]) && options[dtTarget.Rows[i][13].ToString()] == null)
                        {
                            symmetry = options["stone_symmetry"] + dtTarget.Rows[i][13].ToString()[0];
                        }
                        else
                        {
                            continue;
                        ...
                        if (!string.IsNullOrEmpty(shape) && shape == "RND")
                        {
                            ...
                            if (!string.IsNullOrEmpty(dtTarget.Rows[i][11].ToString()) && !string.IsNullOrEmpty(options["stone_cut"]) && options[dtTarget.Rows[i][11].ToString()] == null)
                            {
                                cut_grade = options["stone_cut"] + dtTarget.Rows[i][11].ToString()[0];
                            }
                            else
                            {
                        ...
                        if (!string.IsNullOrEmpty(dtTarget.Rows[i][14].ToString()) && !string.IsNullOrEmpty(options["stone_fluorescence"]) && options[dtTarget.Rows[i][14].ToString()] == null)
                        {
                            fluorescence_intensity = options["stone_fluorescence"] + dtTarget.Rows[i][14].ToString()[0];
                        }
                        else
                        {
                            continue;
                        ...
                        string lab = dtTarget.Rows[i][2].ToString().Replace("LAB", "DF");
                        if (int.Parse(dtTarget.Rows[i][17].ToString()) > 0 && int.Parse(dtTarget.Rows[i][19].ToString()) > 0) { }
                        else
                        {
                            continue;
                        }
                        double carat_price = double.Parse(dtTarget.Rows[i][17].ToString());
                        double total_price = double.Parse(dtTarget.Rows[i][19].ToString());
                        int country = 0;
                        List<string> unavailable_country = null;
                        if (!string.IsNullOrEmpty(countries_list[dtTarget.Rows[i][38].ToString()]))
                        {
                            country = int.Parse(countries_list[dtTarget.Rows[i][38].ToString()]);
                        }
                        else
                        {
                            unavailable_country.Add(dtTarget.Rows[i][38].ToString());
                            country = 259;
                        }
                        ...
                        string measurement = dtTarget.Rows[i][20].ToString() + " x " + dtTarget.Rows[i][21].ToString() + " x " + dtTarget.Rows[i][25].ToString();

                        int measlength = 0;
                        int measwidth = 0;
                        int measdepth = 0;
                        int ratio = 0;
                        measlength = int.Parse(dtTarget.Rows[i][21].ToString());
                        measwidth = int.Parse(dtTarget.Rows[i][20].ToString());
                        measdepth = int.Parse(dtTarget.Rows[i][25].ToString());
                        ratio = measlength / measwidth;

                        string cert = dtTarget.Rows[i][6].ToString();
                        string stock = "";
                        string available = "";
                        string depth = dtTarget.Rows[i][22].ToString();
                        string table_table = dtTarget.Rows[i][23].ToString();
                        ...
                        string cert_url = dtTarget.Rows[i][36].ToString();
                        string hascertfile = !string.IsNullOrEmpty(dtTarget.Rows[i][36].ToString()) ? "1": "0";
                        string hasimagefile = "0";
                        string image_url = "";
                        string video_url = !string.IsNullOrEmpty(dtTarget.Rows[i][4].ToString()) ? dtTarget.Rows[i][4].ToString() : "";
                        string packetID = "";
                        ...
                        _drPrice1_description["source"] = "AKARSH"; // source
